...
namespace KaramMarketingWeb
{
    public partial class CreateReservation : System.Web.UI.Page
    {
        ...
        protected void btnClear_Click(object sender, EventArgs e)
        {
            try
            {
                lblMsg.Style.Value = "display:none";
                lblMessage1.Text = "";
        ...
        protected void btnCancel_Click(object sender, EventArgs e)
        {
            try
            {
                Response.Redirect("~/DashBoardReservation.aspx", false);
            }
            catch (Exception exe)
            {
                ErrorMsg.Style.Value = "display:block";
        ...
        protected void lvProductdetails_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            try
            {
                ...
                if (e.Item.ItemType == ListViewItemType.DataItem)
                {
                    ...
                    Label lblAvailableQty1 = (Label)e.Item.FindControl("lblAvailableQty1");

                    string PackStyle = lblPackStyle.Text;
                    string _ItemCode = lblItemCode.Text;

                    double _AvailQty = double.Parse(lblAvailableQty1.Text);

                    if(_AvailQty <= 0)
                    {
                        _Cls.DeleteCartItem(lblItemCode.Text, UserName);
                    }
                    ...
                    if (!string.IsNullOrWhiteSpace(txtReqPack.Text))
                    {
                        ...
                        if (dllPackStyle.SelectedIndex > 0)
                        {
                            ...
                            if (DemandQty > AvailQty)
                            {
                                txtReqPack.Text = "0";
                                ErrorMsg.Style.Value = "display:block";
                                lblMessage.Text = "Required Quantity is higher than Available Quantity";
                            }
                            else
                            {
        ...
        protected void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (lvProductdetails.Items.Count > 0)
                {
                    ...
                    if (dtdetails != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(NewReservationID))
                        {
                            ...
                            lblMessage1.Text = "Reservation ID Generated : " + NewReservationID;
                            ...
                            {
                                ErrorMsg.Style.Value = "display:block";
                                lblMessage.Text = "Reservation ID Generated successfully, Problem in sending Email";
                            }
                            //BindCartData();
                        ...
                        {
                            ErrorMsg.Style.Value = "display:block";
                            lblMessage.Text = "Error in Generating Reservation ID";
                        }
                    }
                }
                else
                {
                    ErrorMsg.Style.Value = "display:block";
                    lblMessage.Text = "Please Add items in Cart";
                    return;
                }
