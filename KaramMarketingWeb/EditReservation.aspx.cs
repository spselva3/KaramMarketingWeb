    ...
    public partial class EditReservation : System.Web.UI.Page
    {
                    ...
                    if (!string.IsNullOrWhiteSpace(txtReqPack.Text))
                    {
                        ...
                        if (dllPackStyle.SelectedIndex > 0)
                        {
                            ...
                            if (DemandQty > AvailQty)
                            {
                                txtReqPack.Text = "0";
                                ErrorMsg.Style.Value = "display:block";
                                lblMessage.Text = "Demand Qty is higher than Available Qty";
                            }
                            else
                            {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ErrorMsg.Style.Value = "display:block";
                    lblMessage.Text = "Add Items in Cart";
                    return;
                }
                ...
                {
                    ErrorMsg.Style.Value = "display:block";
                    lblMessage.Text = "Add Items with Demand Qty";
                    return;
                }
                _Cls.UpdateSalesOrder(ReserveID, DemandDate, _Customer, _SalerOrder, Status, SODate, UserName.Trim(), txtRemarks.Text);
                ...
                try
                {
                    ...
                    lblMessage1.Text = "Sales Order updated for this Reservation ID" ;

                    try
                    {
                        _Cls.EmailUpdateReservation(ReserveID.Trim());
                    }
                    catch (Exception ex)
                    {
                        ErrorMsg.Style.Value = "display:block";
                        lblMessage.Text = "Reservation ID Updated successfully, Problem in sending Email";
                    }
                }
                catch (Exception exe)
                {
        ...
        protected void btnClose_Click(object sender, EventArgs e)
        {
            try
            {
                Response.Redirect("~/frmMarketingList.aspx", false);
            }
            catch (Exception exe)
            {
                ErrorMsg.Style.Value = "display:block";
            ...
            try
            {
                ...
                if (dtdetails != null)
                {
                    ...
                    if (dtdetails.Rows.Count > 0)
                    {
                        ...
                        foreach (System.Data.DataRow dr in dtdetails.Rows)
                        {
                            ...
                            for (int i = 0; i < lvProductdetails.Items.Count; i++)
                            {
                                ...
                                if (NewItemCode.ToUpper() == _itemCode.ToUpper())
                                {
                                    ...
                                    {
                                        ErrorMsg.Style.Value = "display:block";
                                        lblMessage.Text = "Qty not matched with ItemCode :" + NewItemCode.ToUpper();
                        ...
                        {
                            ErrorMsg.Style.Value = "display:block";
                            lblMessage.Text = "Item Code or Quantity is not matching with Sales Order";
                            return "ItemCode Not Matched with SO";
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        txtCustomer.Text = "";
                        ErrorMsg.Style.Value = "display:block";
                        lblMessage.Text = "Please enter correct Sales Order Number";
                        return "No SO Exist";
                    }
                }
                else
        ...
        protected void btnCancelReservation_Click(object sender, EventArgs e)
        {
            try
            {
                if(ddlReserveStatus.SelectedIndex <= 0)
                {
                    ErrorMsg.Style.Value = "display:block";
                    lblMessage.Text = "Please Select the Cancel Status from drop down ";
                }
                else
                {
                    if (lvProductdetails.Items.Count > 0)
                    {
                        for (int i = 0; i < lvProductdetails.Items.Count; i++)
                        {
                            ...
                            _Cls.CancellReserveItem(txtReservation.Text.Trim(), lblItemCode1.Text, UserName);
                        }
                        lblMsg.Style.Value = "display:block";
                        lblMessage1.Text = "Reservation ID cancelled sucessfully";
                    }
                    else
                    {
                        ErrorMsg.Style.Value = "display:block";
                        lblMessage.Text = "No items to cancel the reservation";
                    }
                }
        ...
        protected void txtSalesOrder_TextChanged(object sender, EventArgs e)
        {
            try
            {
                int len = txtSalesOrder.Text.Length;
                if (len > 5)
                {
                    DataTable dtdetails = new DataTable();
                    dtdetails = _Cls.GetSodetail_Auto(txtSalesOrder.Text.Trim());
                    if (dtdetails != null)
                    {
                        if (dtdetails.Rows.Count > 0)
                        {
                            txtCustomer.Text = dtdetails.Rows[0]["SO_Customer"].ToString();
                        }
                        else
                        {
